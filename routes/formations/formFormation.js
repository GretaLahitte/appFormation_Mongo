...
//          GLOBAL.database_schema.Formations.findById({'_id': req.params._id},function(err, result) {
  GLOBAL.database_schema.Formations.findById({'_id': req.params._id}).populate('formateurs').exec(function(err, result) {
  GLOBAL.database_schema.Formations.findById({'_id': req.params._id}).populate('centres').exec(function(err, resulte) {
    coord=resulte;
